...
require("lazy").setup({
	...
	{ -- Fuzzy Finder (files, lsp, etc)
		...
		config = function()
			...
			vim.keymap.set("n", "go", builtin.diagnostics, { desc = "[S]earch [D]iagnostics" })
	...
	-- copilot
	{
		...
		config = function()
			...
			-- GitHub Copilot
			vim.api.nvim_set_keymap("n", "<F9>", ":lua EnableCopilot()<CR>", { noremap = true, silent = true })
...
vim.api.nvim_set_keymap(
	"n", -- ノーマルモードで適用
	"<Space><Space>", -- スペース2回
	"<cmd>lua vim.lsp.buf.hover()<CR>", -- LSPのホバー機能を呼び出す
	{ noremap = true, silent = true } -- ノンリマップ、サイレント実行
)
...
vim.keymap.set("n", "<leader>fm", builtin.oldfiles, { desc = "Telescope live grep" })
