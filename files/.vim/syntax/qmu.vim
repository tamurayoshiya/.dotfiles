...
syntax region separator start=+--+ end=+$+
syntax keyword func OF MUST IS DEFAULT SHOWS FROM
syntax keyword section behaves has stores pages as belongs seeds for
syntax keyword segment Top Menus Structures
syntax match guard /|/
syntax match model /[A-Z][a-z0-9]*/
syntax keyword group Normal Admin Guest
syntax keyword behave Account Master Photo
syntax match txtString /文字列/
syntax match txtIdentifier /何これ/
syntax match txtError /エラー/

highlight link separator Comment
highlight link func Function
highlight link guard Comment
highlight link section Type
highlight link model Keyword
highlight link behave Identifier
highlight link segment Constant
highlight link group String
highlight link txtIdentifier Identifier
highlight link txtError Error

let b:current_syntax = "qmu"
